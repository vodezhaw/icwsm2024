...
def main():
    ...
    for suff in suffixes_to_plot:
        for q_strat in ['CC', "PACC", "CPCC", "BCC"]:
            groupings[f"{SUBSAMPLING_PREVALENCES[suff]:.3f}", q_strat] = [
                r
                for r in res
                if r.quant_strategy == q_strat and suff in r.scores_file and r.sample_selection_strategy == 'quantile'
            ]
    ...
    for error_type in ["AE", "SAPE"]:
        render_quant_results(
            ...
            save_as=f"paper_plots/low_prevalence/selected_quantile_{error_type}.tex",
        )
